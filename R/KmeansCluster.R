KmeansCluster <-function(Data,FestgesetzteClustAnz=2,Centers=NULL,method='LBG',Verbose=0){
# Cls <- KmeansCluster(Data,FestgesetzteClustAnz,Verbose);
...
if(!is.null(Centers))  
  FestgesetzteClustAnz= Centers  # fuer Backward Compatibility
  
  requireNamespace('cclust')

  if(method=='Hartigan'){
  	c=kmeans(Data,FestgesetzteClustAnz,iter.max = 100);
...
#    requireRpackage('cclust')
    res=cclust::cclust(x=Data,centers=FestgesetzteClustAnz,method='kmeans')
    SSE=res$withinss
